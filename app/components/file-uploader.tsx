...
interface IFileUploader { 
    fieldChange: (file: any) => void, 
    mediaUrl: string[] | File[], 
    extensions?: string[], 
    is_multiple?: boolean, 
    size_limit?: number, 
    file_count?: number, 
    on_change?: (value: string[] | File[]) => void, 
    name?: string, 
    id?: string,
    ref?:React.RefObject<HTMLInputElement|null> 
}

const FileUploader: React.FC<IFileUploader> = ({ fieldChange, mediaUrl, extensions = ['.png', '.jpeg', '.jpg', '.svg', '.webp', '.zip', '.pdf', '.mp4', '.mp3', '.txt'], is_multiple, size_limit, file_count, on_change, name, id, ref }) => {
    ...
    const is_image_file = useCallback(() => {
        let accepted_files = extensions.map(e => e.startsWith('.') ? e : `.${e}`);
        const image_extensions = ['.png', '.jpg', '.jpeg', '.svg', '.webp', '.avif'];
        return accepted_files.some(e=>!image_extensions.includes(e)) ? false : true;    
    }, []);

    const is_image_file_from_extension = useCallback((file: string) => {
    ...
    const removeImage = (index: number) => {
        const filtered = file_count ? mediaUrl.slice(0,file_count).filter((e, i) => i !== index) : mediaUrl.slice().filter((e, i) => i !== index);
    ...
    useEffect(() => {
        ...
        const upload_file = async () => {
            ...
            if (mediaUrl && mediaUrl.length > 0) {
                if(file_count){
                    mediaUrl = mediaUrl.slice(0,file_count);
                }
                if (is_multiple) {
                    const files_array: { is_image: boolean, name: string }[] = [];
                    mediaUrl.forEach((element, index) => {
                        if (element instanceof File) {
                            const is_image = is_image_file_from_extension(element.name);
                    ...
                    if (element instanceof File) {
                        ...
                        const is_image = is_image_file();
                        set_file_url({ is_image: is_image, name: is_image ? element : 'Uploaded' });
                    }
    ...
    return (
        ...
        is_multiple ?
            ...
            <div {...getRootProps()} className="flex justify-start flex-col rounded-xl w-full cursor-pointer">
                <input {...getInputProps()} className="cursor-pointer w-full" name={name} id={id} />
                {file_urls.length > 0 ? (
                    <div className="w-full bg-slate-400 rounded-2xl p-4 overflow-x-auto">
                        <div className="flex flex-row gap-2 w-full">
                            {file_urls.map((file_url, index) => (
                                <div key={index} className="relative w-[150px] h-[150px] flex-shrink-0">
                                    ...
                                    <Button
                                        onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation(); // Prevent dropzone from gettiung triggered
                                            removeImage(index);
                                        }}
                                        variant="default"
                                        size="icon"
                            ...
                            {is_show_upload && <div className="h-[150px] bg-slate-400 rounded-2xl flex flex-col items-center justify-center">
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="100"
                                    height="70"
                    ...
                    <div className="w-full h-[300px] bg-slate-400 rounded-2xl flex flex-col items-center justify-center">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="100"
                            height="70"
            ...
            <div {...getRootProps()} className="flex justify-start flex-col rounded-xl cursor-pointer">
                <input {...getInputProps()} className="cursor-pointer" name={name} />
                {

                    file_url ?
                        <div className="w-full h-[300px] bg-slate-400 rounded-2xl flex flex-col items-center justify-center relative">
