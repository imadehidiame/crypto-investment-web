...
import { $getSelection, $isRangeSelection, CAN_REDO_COMMAND, CAN_UNDO_COMMAND, FORMAT_ELEMENT_COMMAND, FORMAT_TEXT_COMMAND, REDO_COMMAND, SELECTION_CHANGE_COMMAND, UNDO_COMMAND, OUTDENT_CONTENT_COMMAND, INDENT_CONTENT_COMMAND, $getRoot, type SerializedLexicalNode, $createTextNode, $insertNodes, $createParagraphNode, $createLineBreakNode } from 'lexical';
  ...
  useEffect(() => {
    if(insertion && insertion?.trim().length > 0){
      editor.update(()=>{
        ...
        root.clear();
        /*let text = [];
        if(root.getAllTextNodes().length > 0){
          text.push($createLineBreakNode());
        }*/
       const paragraph = $createParagraphNode();
       paragraph.append($createTextNode(insertion));
        //$insertNodes([/*...text,*/$createTextNode(insertion)]);
...
const LexicalTextEditor: React.FC<LexicalProps> = ({ fieldChange, disable, placeholder, value, classNames, name, id, ref, insertion }) => {
  useEffect(()=>{
    //console.log('value has changed in editor');
    //console.log(value);
  },[value]);
  const initialConfig = {
