...
import {
  ...
  export const RRForm = forwardRef<HTMLFormElement, CustomFormProps<any>>(
    (
        ...
        if (state !== "loading" && state !== "submitting") {
          ...
          if (actionData) {
            ...
            if ((!errors && !error) || (logged && logged === true)) {
              if (afterSubmitAction) afterSubmitAction(message, data);
            } else {
              ...
              if (error) {
                ...
                else Toasting.error(error, 10000);
              }
            }
          }
        } else {
      ...
      }, [navigation.state, actionData, form, validateValues, notify, afterSubmitAction, loadAnimation, hideAnimation]);
